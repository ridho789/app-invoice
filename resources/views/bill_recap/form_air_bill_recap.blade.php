        ...
        <div id="form-new-bill-recap">
            <div class="card mb-4">
                ...
                <div class="card-body px-4 pt-0 pb-0">
                    @if ($airBill)
                        <form class="form" action="{{ url('air_bill_recap-update') }}" method="POST">
                            @csrf
                            <div class="form-bill-recap mb-4">
                                <input type="hidden" name="id" value="{{ $airBill->id_air_bill_recap }}">
                                <div class="input-group input-group-static">
                                    ...
                                    <select class="form-control" name="id_customer" disabled>
                                        ...
                                        <option value="{{ $c->id_customer }}" {{ old('id_customer', $airShipment->id_customer) == $c->id_customer ? 'selected' : '' }}>{{ $c->name }}
                                        </option>
                                        @endforeach
                                    </select>
                                    @else
                                ...
                                <div class="input-group input-group-static">
                                    ...
                                    <input type="date" name="load_date" class="form-control" value="{{ old('load_date', $airShipment->date) }}" disabled>
                                </div>

                                <div class="input-group input-group-static">
                                    <label class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">No. Inv</label>
                                    <input type="text" name="no_inv" class="form-control" value="{{ old('no_inv', $airBill->inv_no) }}" disabled>
                                </div>

                                <div class="input-group input-group-static">
                                    <label class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Freight</label>
                                    <input type="text" name="freight" class="form-control" value="{{ old('freight', $airBill->freight_type) }}" disabled>
                                </div>

                                <div class="input-group input-group-static">
                                    <label class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Total Units</label>
                                    <input type="text" name="size" id="size" class="form-control" value="{{ old('size', $airBill->size) }}" disabled>
                                </div>

                                <div class="input-group input-group-static">
                                    <label class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Unit Price</label>
                                    <input type="text" name="unit_price" id="unit_price" class="form-control" 
                                    value="{{ old('unit_price', 'Rp ' . number_format($airBill->unit_price ?? 0, 0, ',', '.')) }}" disabled>
                                </div>

                                <div class="input-group input-group-static">
                                    <label class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Amount</label>
                                    <input type="text" name="amount" id="amount" class="form-control" 
                                    value="{{ old('amount', 'Rp ' . number_format($airBill->amount ?? 0, 0, ',', '.')) }}" disabled>
                                </div>

                                <div class="input-group input-group-static">
                                    <label class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Payment Date</label>
                                    <input type="date" name="payment_date" value="{{ old('payment_date', $airBill->payment_date) }}" class="form-control" required>
                                </div>

                                <div class="input-group input-group-static">
                                    <label class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Payment Amount</label>
                                    <input type="text" name="payment_amount" id="payment_amount" value="{{ old('payment_amount', $airBill->payment_amount) }}" class="form-control">
                                </div>
                                

                                <div class="input-group input-group-static">
                                    <label class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Remaining Bill</label>
                                    <input type="text" name="remaining_bill" id="remaining_bill" value="{{ old('remaining_bill', $airBill->remaining_bill) }}" class="form-control" readonly>
                                </div>

                                <div class="input-group input-group-static">
                                    <label class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Overdue Bill</label>
                                    <input type="date" name="overdue_bill" value="{{ old('overdue_bill', $airBill->overdue_bill) }}" class="form-control">
                                </div>
                            </div>
